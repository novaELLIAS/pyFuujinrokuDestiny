...
class EnemyBase(object):
    ...
    def update(self, enemy):
        ...
        if enemy.status == globe.enstatus["dead"] and (not globe.BOSSING):
            # 被击破, 画bubble动画, 删除敌机
            enemy.status = globe.enstatus["del"]
            globe.scgame.anManager.create_anime(globe.destiny.rsManager.anime["bubble"], enemy.rect.topleft, 5)
        else:
            if not globe.playrc.inflate(64, 64).collidepoint(self.point):
                # 敌机越界, 删除敌机
                enemy.status = globe.enstatus["del"]
        self.update_orbit()       # 更新敌机轨迹
        self.update_enemy(enemy)  # 更新敌机状态
        self.create_bullet(self)  # 更新弹幕
        enemy.frame += 1          # 帧计数
